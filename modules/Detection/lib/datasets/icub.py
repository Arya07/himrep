...
import subprocess
import uuid
from icub_eval import icub_eval
...
class icub_dataset(imdb):
    def __init__(self, image_set, devkit_path=None):
        imdb.__init__(self, 'iCubWorld-Transformation')
        self._image_set = image_set
        self._devkit_path = self._get_default_path() if devkit_path is None \
                            else devkit_path
        self._data_path = os.path.join(self._devkit_path, 'data')
        self._classes = ('__background__', # always index 0
                         'book1', 'flower4', 'glass8', 'hairclip8',
                         'hairbrush1','sunglasses4', 'sodabottle2',
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'iCubWorld-Translation_devkit')
    ...
    def _do_python_eval(self, output_dir = 'output'):
        ...
        if not os.path.isdir(output_dir):
            os.mkdir(output_dir)
        with open('mAP_10cl_1ins_4tr_80-20_scale_300prop_th04.txt', 'wt') as f1:
            for i, cls in enumerate(self._classes):
                if cls == '__background__':
                    continue
                filename = self._get_icub_results_file_template().format(cls)
                rec, prec, ap = icub_eval(
                    filename, annopath, imagesetfile, cls, cachedir, ovthresh=0.4)
                aps += [ap]
                precs += [prec]
